...
export async function deleteImageById(id) {
  try {
    ...
    if (
      session.user.id === JSON.parse(JSON.stringify(image.userId)) ||
      ...
      if (response.result === "ok") {
        ...
        await Like.deleteMany({ photoId: image._id });
        await Comment.deleteMany({ photoId: image._id });
...
export async function editImageById(id, formData) {
  try {
    ...
    await dbConnect();
    const session = await getServerSession(authOptions);
    let image = await Photo.findById(id);
    if (!session || session.user.id != image.userId) {
      return {
        success: false,
        message: "Access forbidden",
        statusCode: 403,
    ...
    if (session.user.id === JSON.parse(JSON.stringify(image.userId))) {
      ...
      if (formData.albumId) image.albumId = formData.albumId;
      else image.albumId = null;
      ...
      return {
        ...
        image: JSON.parse(JSON.stringify(updatedImage)),
        statusCode: 200,
      };
    } else {
      return {
...
export async function getPhotosWithoutAlbum() {
  try {
    await dbConnect();
    const session = await getServerSession(authOptions);
    if (!session)
    ...
    const photos = await Photo.find({ userId: session.user.id, albumId: null });
    return {
      success: true,
      photos: JSON.parse(JSON.stringify(photos)),
      statusCode: 200,
...
export async function getPhotos(skip, limit) {
  try {
    ...
    return {
      success: true,
      images: JSON.parse(JSON.stringify(transformedImages)),
      statusCode: 200,
    };
  } catch (error) {
    console.error(error);
...
export async function searchPhoto(text) {
  try {
    await dbConnect();
    const hashtags =
    ...
    if (textWithoutHashtags) {
      query.$or.push({
        title: { $regex: textWithoutHashtags, $options: "i" },
      });
      query.$or.push({
        description: { $regex: textWithoutHashtags, $options: "i" },
      });
    }
...
export async function deleteComment(commentId, photoId) {
  try {
    ...
    const session = await getServerSession(authOptions);
    if (
      session.user.id === JSON.parse(JSON.stringify(comment.userId)) ||
      session.user.role == "admin"
    ) {
